...
impl SwapChain {
    fn new(
        ...
        let swap_chain: IDXGISwapChain3 = swap_chain1
            .cast()
            .map_platform_error("unable to cast swap chain 1 to v3")?;
    ...
    fn render_and_present<T>(
        ...
        unsafe {
            self.command_queue
                .Signal(&self.fence, self.fence_values[self.current_buffer_index])
        }
    ...
    fn wait_for_buffer(&mut self, buffer_index: usize) -> Result<(), PlatformError> {
        if unsafe { self.fence.GetCompletedValue() } < self.fence_values[buffer_index] {
            unsafe {
                self.fence
                    .SetEventOnCompletion(self.fence_values[buffer_index], self.fence_event)
            }
