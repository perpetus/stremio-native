...
impl super::Surface for MetalSurface {
    fn new(
        ...
        let gr_context = skia_safe::gpu::direct_contexts::make_metal(&backend, None)
            .unwrap()
            .into();
    ...
    fn render(
        ...
        autoreleasepool(|_| {
            ...
            let age = {
                ...
                if let Some(existing_age) = drawables
                    .iter()
                    .find_map(|(id, age)| (*id == texture_id).then_some(*age))
                {
                    existing_age
                } else {
