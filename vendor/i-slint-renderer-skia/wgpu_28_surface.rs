    ...
    pub(crate) fn flush_and_submit(&self, gr_context: &mut skia_safe::gpu::DirectContext) {
        ...
        if !textures_to_transition.is_empty() {
            ...
            encoder.transition_resources(
                std::iter::empty(),
                textures_to_transition
                    .iter()
                    .map(|texture| wgpu::TextureTransition {
                        texture,
                        selector: None,
...
impl crate::Surface for WGPUSurface {
    ...
    fn import_wgpu_texture(
        ...
        // submitting.
        self.textures_to_transition_for_sampling
            .borrow_mut()
            .push(texture.clone());
...
impl TryFrom<wgpu::Backend> for Backend {
    ...
    fn try_from(wgpu_backend: wgpu::Backend) -> Result<Self, Self::Error> {
        match wgpu_backend {
            wgpu_28::Backend::Noop => Err(PlatformError::from(
                "Cannot use WGPU Noop backend with Skia",
            )),
            #[cfg(all(target_family = "unix", not(target_vendor = "apple")))]
