...
use crate::animations::physics_simulation::{
    ConstantDecelerationParameters, CubicEaseOutParameters,
};
...
///
/// STREMIO-NATIVE PATCH: upstream default is 2000, which stops a 1000px/s fling
/// after 250px. Browsers decelerate at 980-1200 and glide 416px from the same
/// release, which is the difference between "stiff" and "fluid".
const DECELERATION: f32 = 1200.;
...
///
/// STREMIO-NATIVE PATCH: upstream default is 180ms. Chromium and Firefox both
/// land in the 200-250ms band for a wheel notch.
const WHEEL_SCROLL_DURATION: Duration = Duration::from_millis(250);
/// Scale applied to a notched mouse wheel step.
///
/// STREMIO-NATIVE PATCH: the winit backend hands us 60px per wheel line
/// (`lx * 60.` in i-slint-backend-winit), while browsers scroll a 100px notch
/// (3 lines x 33.3px). Correcting it here rather than in the backend keeps the
/// scale next to the branch that already knows this is a notched wheel and not
/// a touchpad; a touchpad reports pixel deltas through the phased path below
/// and must stay 1:1 with the fingers.
const WHEEL_NOTCH_SCALE: f32 = 100. / 60.;
...
struct FlickableDataInner {
    ...
    running_animation: Option<(Instant, [Option<CubicEaseOutParameters>; 2])>,
}
...
impl FlickableDataInner {
    ...
    /// Whether this event is a step of a notched mouse wheel rather than part of
    /// a phased touchpad gesture.
    ///
    /// STREMIO-NATIVE PATCH: a touchpad opens its gesture with `Started`, which
    /// takes the capture; a notched wheel only ever reports `Moved`. The two
    /// need different treatment - a notch is an impulse to animate, a touchpad
    /// delta is a finger position to follow exactly.
    fn is_notched_wheel_step(capture_events: Option<CaptureEvents>, phase: TouchPhase) -> bool {
        phase == TouchPhase::Moved
            && !capture_events.is_some_and(|capture| capture == CaptureEvents::MouseWheel)
    }
    ...
    fn process_wheel_event(
        ...
        // STREMIO-NATIVE PATCH: bring a notched wheel step up to the browser's
        // 100px. This is the same test the animation branch below uses to tell a
        // notched wheel apart from a phased touchpad gesture, and it has to run
        // before the delta is clamped so the bounds are computed against the
        // real target.
        if Self::is_notched_wheel_step(self.capture_events, phase) {
            delta.x = (delta.x as f32 * WHEEL_NOTCH_SCALE) as Coord;
            delta.y = (delta.y as f32 * WHEEL_NOTCH_SCALE) as Coord;
        }
        ...
        match phase {
            ...
            TouchPhase::Moved => {
                if self.capture_events.is_some_and(|capture| capture == CaptureEvents::MouseWheel) {
                    ...
                    //
                    // STREMIO-NATIVE PATCH: the curve is a cubic ease-out rather
                    // than upstream's constant deceleration, matching the browser
                    // impulse response. `CubicEaseOut` integrates incrementally
                    ...
                    let x_simulation = (delta.x != Coord::default()).then(|| {
                        let simulation = CubicEaseOutParameters::new_with_distance(
                            delta.x as f32,
