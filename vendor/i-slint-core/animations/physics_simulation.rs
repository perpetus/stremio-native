...
impl ConstantDecelerationParameters {
    ...
    #[expect(
        dead_code,
        reason = "STREMIO-NATIVE PATCH: the wheel now uses CubicEaseOut, leaving this upstream constructor unused. Kept verbatim so re-vendoring a new Slint release stays a clean diff."
    )]
    ...
    #[expect(
        dead_code,
        reason = "STREMIO-NATIVE PATCH: superseded by CubicEaseOutParameters::remaining_distance on the wheel path. Kept verbatim so re-vendoring stays a clean diff."
    )]
...
/// Input parameters for the `CubicEaseOut` simulation.
///
/// STREMIO-NATIVE PATCH: added to give mouse wheel scrolling the same impulse
/// response as a browser. `ConstantDeceleration` traces `1 - (1 - u)^2`, which
/// leaves the wheel feeling abrupt; Chromium and friends trace
/// `1 - (1 - u)^3`, the cubic-bezier(0.25, 0.1, 0.25, 1.0) approximation, which
/// starts 50% faster and settles with a longer tail.
/// See `vendor/i-slint-core/PATCHES.md`.
#[derive(Debug, Clone)]
pub struct CubicEaseOutParameters {
    /// Total distance the animation covers over its whole duration.
    pub distance: f32,
    /// Duration of the animation in seconds.
    pub duration_secs: f32,
}

impl CubicEaseOutParameters {
    /// * `distance` - the distance to cover with this animation
    /// * `duration_secs` - the duration of the animation in seconds
    pub fn new_with_distance(distance: f32, duration_secs: f32) -> Self {
        debug_assert!(duration_secs > 0., "Duration must be greater than zero");
        Self { distance, duration_secs }
    }

    /// Fraction of the total distance already travelled at `time_elapsed`.
    ///
    /// This is the normalized cubic ease-out `s(u) = 1 - (1 - u)^3`, clamped to
    /// the animation window so callers may query it past the end.
    fn progress(&self, time_elapsed: f32) -> f32 {
        if time_elapsed >= self.duration_secs {
            return 1.;
        }
        let remaining = 1. - (time_elapsed / self.duration_secs).max(0.);
        1. - remaining * remaining * remaining
    }

    /// Calculates the distance still to travel at a given time.
    ///
    /// The Flickable folds this into the next wheel event so that a burst of
    /// notches accumulates onto the pending target instead of restarting from
    /// the animated position, which is what makes fast scrolling track the
    /// input 1:1 the way a browser does.
    pub fn remaining_distance(&self, time_elapsed: core::time::Duration) -> Coord {
        (self.distance * (1. - self.progress(time_elapsed.as_secs_f32()))) as Coord
    }
}

impl Parameter for CubicEaseOutParameters {
    type Output = CubicEaseOut;
    fn simulation(
        self,
        start_value: f32,
        limit_value: core::pin::Pin<alloc::boxed::Box<crate::Property<f32>>>,
    ) -> Self::Output {
        CubicEaseOut::new(start_value, limit_value, self)
    }
}

/// Traces a cubic ease-out from the position the property held when the
/// animation started, stopping early if it runs into `limit_value`.
///
/// Like `ConstantDeceleration` this integrates *incrementally* rather than
/// assigning an absolute position each tick. That is a hard requirement: a
/// ListView rewrites `viewport-y` from its own layout pass whenever it revises
/// a size estimate, and an absolute curve would fight those corrections and
/// visibly stutter. Applying only the per-frame delta lets both writers
/// compose.
#[derive(Debug)]
pub struct CubicEaseOut {
    /// Value at which the simulation ends if the curve did not complete first.
    limit_value: core::pin::Pin<alloc::boxed::Box<crate::Property<f32>>>,
    data: CubicEaseOutParameters,
    direction: Direction,
    start_time: Instant,
    /// Seconds elapsed at the previous step, so each tick contributes the
    /// difference between two points on the curve.
    previous_elapsed: f32,
}

impl CubicEaseOut {
    /// * `start_value` - start position
    /// * `limit_value` - value at which the simulation ends if reached early
    /// * `data` - the properties of this simulation
    pub fn new(
        start_value: f32,
        limit_value: core::pin::Pin<alloc::boxed::Box<crate::Property<f32>>>,
    ...
    fn new_internal(
        ...
        let direction = if data.distance >= 0. {
            debug_assert!(start_value <= limit_value.as_ref().get() + f32::EPSILON);
            Direction::Increasing
        } else {
            Direction::Decreasing
        };

        Self { limit_value, data, direction, start_time, previous_elapsed: 0. }
    }

    fn step_internal(&mut self, current: &mut f32, new_tick: Instant) -> bool {
        let elapsed = new_tick.duration_since(self.start_time).as_secs_f32();
        let progress = self.data.progress(elapsed);
        let previous_progress = self.data.progress(self.previous_elapsed);
        self.previous_elapsed = elapsed;

        *current += self.data.distance * (progress - previous_progress);

        let limit_value = self.limit_value.as_ref().get();
        match self.direction {
            Direction::Increasing if *current >= limit_value => {
                *current = limit_value;
                return true;
            }
            Direction::Decreasing if *current <= limit_value => {
                *current = limit_value;
                return true;
            }
            _ => {}
        ...
        elapsed >= self.data.duration_secs
    }
}

impl Simulation for CubicEaseOut {
    fn step(&mut self, current: &mut f32, new_tick: Instant) -> bool {
        self.step_internal(current, new_tick)
    }
}
...
// STREMIO-NATIVE PATCH: coverage for the browser-style wheel curve.
#[cfg(test)]
mod tests_cubic_ease_out {
    use super::*;
    use core::time::Duration;
    ...
    #[test]
    fn cubic_ease_out_covers_the_whole_distance() {
        const START_VALUE: f32 = 0.;
        const DISTANCE: f32 = -100.;

        let mut time = Instant::now();
        let mut simulation = CubicEaseOut::new_internal(
            START_VALUE,
            test_limit_property(-1000.),
            CubicEaseOutParameters::new_with_distance(DISTANCE, DURATION_SECS),
            time,
        );
        let mut current = START_VALUE;

        time += Duration::from_millis(250);
        assert!(simulation.step(&mut current, time));
        assert_approx_eq!(current, DISTANCE);
    }

    #[test]
    fn cubic_ease_out_front_loads_the_distance() {
        const START_VALUE: f32 = 0.;
        const DISTANCE: f32 = -100.;
        ...
        // s(0.5) = 1 - 0.5^3 = 0.875: the browser curve is well past halfway at
        // the halfway point, which is what makes it read as responsive.
        time += Duration::from_millis(125);
        assert!(!simulation.step(&mut current, time));
        assert_approx_eq!(current, DISTANCE * 0.875);
    }

    #[test]
    fn cubic_ease_out_stops_at_the_limit() {
        const START_VALUE: f32 = 0.;
        const LIMIT_VALUE: f32 = -30.;

        let mut time = Instant::now();
        let mut simulation = CubicEaseOut::new_internal(
            START_VALUE,
            test_limit_property(LIMIT_VALUE),
            CubicEaseOutParameters::new_with_distance(-100., DURATION_SECS),
            time,
        );
        let mut current = START_VALUE;
    ...
    #[test]
    fn cubic_ease_out_absorbs_external_writes() {
        // A ListView revising its size estimate writes viewport-y underneath a
        // running animation. Each tick must contribute only its own delta so
        // the correction survives instead of being snapped away.
        const DISTANCE: f32 = -100.;
        ...
        let external_correction = -17.;
        current += external_correction;

        time += Duration::from_millis(125);
        assert!(simulation.step(&mut current, time));
        assert_approx_eq!(current, DISTANCE + external_correction);
    }

    #[test]
    fn remaining_distance_shrinks_along_the_curve() {
        let parameters = CubicEaseOutParameters::new_with_distance(-100., DURATION_SECS);

        assert_approx_eq!(parameters.remaining_distance(Duration::ZERO), -100.);
        assert_approx_eq!(parameters.remaining_distance(Duration::from_millis(125)), -12.5);
        assert_eq!(parameters.remaining_distance(Duration::from_millis(250)), 0.);
        assert_eq!(parameters.remaining_distance(Duration::from_secs(10)), 0.);
    }
}
